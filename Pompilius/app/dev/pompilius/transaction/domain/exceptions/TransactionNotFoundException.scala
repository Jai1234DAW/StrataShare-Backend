...
class TransactionNotFoundException(message: String = "Resource not found") extends VerboseException(message = message)

object TransactionNotFoundException {
  def apply(transactionId:TransactionId): TransactionNotFoundException = {
    new TransactionNotFoundException(s"Sample with id=${transactionId.toString} not found")
  }
}