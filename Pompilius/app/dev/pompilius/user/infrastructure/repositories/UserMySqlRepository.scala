...
import dev.pompilius.user.domain.exceptions.UserNotFoundException
import dev.pompilius.user.domain.{User, UserFilter, UserId, UserRepository}
...
class UserMySqlRepository @Inject() (
  ...
  def apply(u: ResultName[User])(rs: WrappedResultSet): User =
    User(
      ...
      avatar = rs.get[Option[Long]](u.avatar).map(ImageId(_)),
