    ...
    Action.async(parse.multipartFormData) { implicit request =>
      withAnyOfThisRoles(Seq(Role.STUDENT, Role.PROFESSIONAL)) {
        case (_, user, _, _) =>
          ...
          // 🚫 Bloquear si viene alguna imagen
          val imageFiles = body.files.filter(
            _.contentType.exists(_.startsWith("image/"))
          )

          if (imageFiles.nonEmpty) {
            Future.successful(
              BadRequest("Images are not allowed in /attachments/files")
            )
          } else {
          ...
          withAuthenticatedUser {
            case (_, user, _) =>
              ...
              for {
                ...
                _ <- resourceAccessValidator.validateAccess(rid, user.id)
          ...
          withAuthenticatedUser {
            case (_, user, _) =>
              ...
              for {
                ...
                allAttachments <- attachmentRepository.findByResourceId(rid, pag.oneMore)
